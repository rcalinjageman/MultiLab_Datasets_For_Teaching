...
# Packages ---------------------------------------
  ...
  if (!is.element("stringr", installed.packages()[,1])) install.packages("stringr", dep = TRUE)
...
# Parameters --------------------------
  # Paths info
  path_original <- "./original_data/ManyLabs3"
  path_extract <- "ML3 Final Data/ML3AllSitesandmTurk.csv"
  path_datafile <- "/ML3AllSitesandmTurk.csv"
  path_prepped <- "./prepped_data"
  path_sites <- "/manylabs3 - sites.csv"
...
# Study definitions ---------------------
  ...
  studies$powerperspective <- list(iv = "PowerCond",
                              dv = "Sincerity",
                              iv_levels = c("HighPower", "LowPower"),
                              ylab = "Predicted interpretation (1 (very scarstic) to 6 (very sincere))\nhigher scores -> better perspective-taking",
                              reference_group = 2,
                              exclude = NULL,
                              keeps = c("age", "year", "Genderfactor", "ethnicity", "AttentionCheck"),
                              name = "ML3 - Power and perspectives not taken",
                              type = "eimd",
                              type_path = "/Estimate Independent Mean Difference",
  ...
  studies$moralcredit <- list(iv = "CredCond",
                              dv = "mcdv1",
                              iv_levels = c("Credentials", "NoCredentials"),
                              ylab = "Rating of job position: -3 (much better for a woman) to +3 (much better for a man)",
                              reference_group = 2,
                              exclude = NULL,
                              keeps = c("age", "year", "Genderfactor", "ethnicity", "AttentionCheck"),
                              name = "ML3 - Moral credentials and expressions of prejudice",
                              type = "eimd",
                              type_path = "/Estimate Independent Mean Difference",
                              path = "/ML3-Moral_Credentials_Prejudice")
...
# Many labs 3 has their data file posted to the OSF, zipped and in csv format
  # NOTE: Have to download file manually--on my windows machines this often leaves file that R cannot unzip
  # download.file("https://osf.io/bxw8j/download", paste(path_original, "/ML3 Final Data.zip", sep=""), cacheOK = FALSE)
  unzip(zipfile = paste(path_original, "/ML3 Final Data.zip", sep = ""), 
        files = path_extract,
        exdir = path_original,
        overwrite = TRUE,
        junkpaths = TRUE)
...
# Process/clean the data ----------------------
  ...
  # Change site to referrer
  data$referrer <- data$Site
  
  # Change sarcasm to sincerity
  data$Sincerity <- data$sarcasm

  # Fix levels for metaphoric restructuring
  levels(data$SRCondition) <- c("", "ego-centric", "object-centric", "control")
  levels(data$SRMeetingResponse) <- c(levels(data$SRMeetingResponse), "Ego-Consistent", "Ego-Inconsistent")
  data[data$SRMeetingResponse %in% c("Friday"), ]$SRMeetingResponse <- "Ego-Consistent"
  data[data$SRMeetingResponse %in% c("Monday"), ]$SRMeetingResponse <- "Ego-Inconsistent"

  # Fix levels for ethnicity
  levels(data$ethnicity)[levels(data$ethnicity) == "1"] <- "African-American"
  levels(data$ethnicity)[levels(data$ethnicity) == "2"] <- "Asian-American"
  levels(data$ethnicity)[levels(data$ethnicity) == "3"] <- "Native Hawaiian and other Pacific Islander"
  levels(data$ethnicity)[levels(data$ethnicity) == "4"] <- "Latino or Hispanic"
  levels(data$ethnicity)[levels(data$ethnicity) == "5"] <- "Native American and Alaska Native"
  levels(data$ethnicity)[levels(data$ethnicity) == "6"] <- "White, non-Hispanic or latino"
  levels(data$ethnicity)[levels(data$ethnicity) == "7"] <- "Multiracial"
  levels(data$ethnicity)[levels(data$ethnicity) == "8"] <- "Other"
  levels(data$ethnicity)[9:length(levels(data$ethnicity))] <- "Other"
  
  # Fix levels for year
  levels(data$year)[levels(data$year) == "1"] <- "First-year"
  levels(data$year)[levels(data$year) == "2"] <- "Sophomore"
  levels(data$year)[levels(data$year) == "3"] <- "Junior"
  levels(data$year)[levels(data$year) == "4"] <- "Senior"
  levels(data$year)[!levels(data$year) %in% c("First-year", "Sophomore", "Junior", "Senior")] <- "Other"
  
  # Fix clipboardweight
  data$ClipboardWeight <- as.factor(data$ClipboardWeight)
  levels(data$ClipboardWeight)[levels(data$ClipboardWeight) == "10"] <- "Light Clipboard"
  levels(data$ClipboardWeight)[levels(data$ClipboardWeight) == "20"] <- "Heavy Clipboard"
  
  # Fix cbreject
  data$CBReject <- as.factor(data$CBReject)
  levels(data$CBReject)
  levels(data$CBReject)[levels(data$CBReject) == "0"] <- "Include"
  levels(data$CBReject)[levels(data$CBReject) == "1"] <- "Exclude"
  
  # Fix IIResponse
  data$IIResponse <- as.integer(data$IIResponse)
  
  # Fix temperature response and temp in lab
  regexp <-"[[:digit:]]+"
  data$tempestclean <- str_extract(data$tempest1,regexp)
  data$tempestclean <- as.numeric(data$tempestclean)
  data[data$tempestclean >= 95 & !is.na(data$tempestclean), ]$tempestclean <- NA
  data[data$tempestclean <= 50 & !is.na(data$tempestclean), ]$tempestclean <- NA
  data$Temperatureinlab <- as.numeric(str_extract(data$Temperatureinlab,regexp))
...
# Loop through studies -----------------------
  for (cstudy in studies) {
    ...
    keeps <- c("Participant_ID", "referrer", cstudy$iv, cstudy$dv, cstudy$exclude, cstudy$keeps)
    ...
    # Clean up data-------------------
      ...
      if(!is.null(cstudy$exclude)) {
        study_data <- study_data[study_data[[cstudy$exclude]] %in% c("Include", "include", "Yes"), ]
      }
