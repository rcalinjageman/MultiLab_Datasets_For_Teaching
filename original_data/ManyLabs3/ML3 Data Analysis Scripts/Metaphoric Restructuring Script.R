...
#Moderation by attention check
Meta$AttentionCheck<-as.factor(Meta$AttentionCheck)
PrimeM.glm<-glm(PrimeCons~DaysInLab*AttentionCheck,data=Meta,family=binomial)
Anova(PrimeM.glm,type="II")
...
SRPrimeCorrect$AttentionCheck<-as.factor(SRPrimeCorrect$AttentionCheck)
AttentionCheck<-glm(PrimeCons~AttentionCheck,data=SRPrimeCorrect,family=binomial)
Anova(AttentionCheck,type="II")
chies(1.305,1335)
...
PLU<-subset(SRPrimeCorrect,Site=="PacificLutheranUniversity")
head(PLU)
str(PLU$PrimeCons)
PLU.glm<-glm(PrimeCons~DaysInComp,data=PLU,family=binomial)
Anova(PLU.glm,type="II")