...
str(PowerData$AttentionCheck)
PowerData$AttentionCheck<-as.factor(PowerData$AttentionCheck)
Attention<-lm(sarcasm~PowerCond*AttentionCheck,data=PowerData)
Anova(Attention,type="II")
plot(allEffects(Attention))
modelEffectSizes(Attention)
ci.pvaf(F.value= 0.8584,df.1=1,df.2= 2883,N=2887,conf.level=.95)
...
head(PLU)
PLU.lm<-lm(sarcasm~PowerCond*DaysInComp,data=PLU)
