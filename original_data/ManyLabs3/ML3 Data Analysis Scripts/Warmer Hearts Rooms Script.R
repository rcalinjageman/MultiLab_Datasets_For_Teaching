...
#Moderation with attention check
Pool$AttentionCheck<-as.factor(Pool$AttentionCheck)
Temp.lm<-lm(tempestclean~TempCond*DaysInComp*AttentionCheck,data=Pool)
Anova(Temp.lm,type="II")
...
TempClean$AttentionCheck<-as.factor(TempClean$AttentionCheck)
AttentionCheck<-lm(tempestclean~TempCond*AttentionCheck,data=TempClean)
Anova(AttentionCheck,type="II")
modelEffectSizes(AttentionCheck)
ci.pvaf(F.value= 1.6623,df.1=1,df.2=2999,N=3003,conf.level=.95)
...
TempSum
###summarise command seems to be having trouble, trying another command
summaryBy(tempestclean~TempCond*Site,FUN = c(mean,sd,length),data = TempClean)
...
PLU<-subset(TempClean,Site=="PacificLutheranUniversity")
head(PLU)
PLU.lm<-lm(tempestclean~TempCond*DaysInComp,data=PLU)
