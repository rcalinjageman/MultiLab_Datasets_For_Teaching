...
setwd("/Users/Charlie/Desktop/ML3 Misc./ML3 Data Analysis/")
...
range(JustTest$trial_latency)

###Performing Log Transformed Mean difference analysis
head(JustTest)
JustTest<-subset(JustTest,trial_latency>0)
JustTest$LogLat<-log(JustTest$trial_latency)
Con<-subset(JustTest,congruent=="Congruent")
Incon<-subset(JustTest,congruent=="Incongruent")
Table<-summarise(group_by(Con,session_id),meanCon=mean(LogLat,na.rm=TRUE))
head(Table)
Table2<-summarise(group_by(Incon,session_id),meanIncon=mean(LogLat,na.rm=TRUE))
head(Table2)
Total<-merge(Table,Table2,by="session_id",all=TRUE)
head(Total)
t.test(Total$meanCon,Total$meanIncon,paired=TRUE,alternative="two.sided",var.equal=TRUE)
Total$Difference<-Total$meanCon-Total$meanIncon
mean(Total$Difference,na.rm=TRUE)/sd(Total$Difference,na.rm=TRUE)
...
#Moderation of attention check
ML3$AttentionCheck<-as.factor(ML3$AttentionCheck)
Stroop.lm<-lm(StroopEffect~DaysInComp*AttentionCheck,data=Pool)
Anova(Stroop.lm,type="II")
...
require(lmSupport)
ML3$AttentionCheck<-as.factor(ML3$AttentionCheck)
AttentionCheck<-lm(StroopEffect~AttentionCheck,data=ML3)
Anova(AttentionCheck,type="II")
modelEffectSizes(AttentionCheck)
ci.pvaf(F.value= 0.0963,df.1=1,df.2=3183,N=3185,conf.level=.95)
...
StroopSum
summaryBy(StroopEffect~Site,FUN = c(mean,sd),data=ML3,na.rm=TRUE)
setwd("/Users/Charlie/Desktop")
...
head(PLU)
PLU.lm<-lm(StroopEffect~DaysInComp,data=PLU)
