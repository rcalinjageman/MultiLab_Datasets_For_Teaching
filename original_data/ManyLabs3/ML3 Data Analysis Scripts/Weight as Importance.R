...
WeightClean$AttentionCheck<-as.factor(WeightClean$AttentionCheck)
AttentionCheck<-lm(IIResponse~ClipboardWeight*AttentionCheck,data=WeightClean)
Anova(AttentionCheck,type="II")
...
PLU<-subset(WeightClean,Site=="PacificLutheranUniversity")
head(PLU)
PLU.lm<-lm(IIResponse~ClipboardWeight*DaysInComp,data=PLU)
