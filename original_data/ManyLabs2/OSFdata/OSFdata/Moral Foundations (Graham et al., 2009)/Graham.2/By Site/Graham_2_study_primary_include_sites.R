...
##
## Moral Foundations (Graham et al., 2009): Graham_2_study_primary_include_sites
##
...
analysis.unique.id     <- 15
analysis.name          <- 'Graham.2'
analysis.type          <- 3
...
ML2.df <- ML2.df  %>% dplyr::select(1,6,129,130,131,132,133,134,430,520,521,522,523,524,525,526,527,528,529,530,531,534,535,536) %>% dplyr::filter(is.character(source))
...
# To see the function code type:varfun.Graham.2, or lookup in manylabRs_SOURCE.R
ML2.var[[g]] <- varfun.Graham.2(ML2.sr[[g]])
...
stat.test   <- try.CATCH(with(ML2.var[[g]],cor_test_fisherZ(r1=matrix(c(Individual,Politics),ncol=2), r2=NULL, n1=N[1], n2=NULL, conf.level=stat.params$conf.level, alternative = stat.params$alternative)))
