...
##
## Direction & Similarity (Tversky & Gati, 1978): Gati_8_study_primary_include_sites
##
...
analysis.unique.id     <- 90
analysis.name          <- 'Gati.8'
analysis.type          <- 3
...
# To see the function code type:varfun.Gati.4, or lookup in manylabRs_SOURCE.R
ML2.var[[g]] <- varfun.Gati.4(ML2.sr[[g]])
...
stat.test   <- try.CATCH(with(ML2.var[[g]],t.test(Asymmetry ~ Order,conf.level=stat.params$conf.level, var.equal = stat.params$var.equal, alternative = stat.params$alternative)))
