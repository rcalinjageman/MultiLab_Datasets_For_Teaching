...
##
## Direction & Similarity (Tversky & Gati, 1978): Gati_6_study_primary_include_sites
##
...
analysis.unique.id     <- 88
analysis.name          <- 'Gati.6'
analysis.type          <- 3
...
ML2.df <- ML2.df  %>% dplyr::select(1,6,656,657,658,659,660,661,662,663,664,665,666,667,668,669,670,671,672,673,674,675,676,682,683,684,685,686,687,688,689,690,691,692,693,694,695,696,697,698,699,700,701,702,804,903,904,905,906,907,908,909,910,911,912,913,914,937,938,939) %>% dplyr::filter(is.character(source))
...
# To see the function code type:varfun.Gati.5, or lookup in manylabRs_SOURCE.R
ML2.var[[g]] <- varfun.Gati.5(ML2.sr[[g]])
...
stat.test   <- try.CATCH(with(ML2.var[[g]],t.test(DV ~ Condition, conf.level=stat.params$conf.level, var.equal = stat.params$var.equal, alternative = stat.params$alternative, paired = TRUE)))
