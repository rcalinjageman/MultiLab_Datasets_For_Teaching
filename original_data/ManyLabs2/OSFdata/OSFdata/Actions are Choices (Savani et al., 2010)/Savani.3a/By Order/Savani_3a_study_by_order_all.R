...
##
## Actions are Choices (Savani et al., 2010): Savani_3a_study_by_order_all
##
...
study.description      <- 'Actions are Choices (Savani et al., 2010)'
analysis.unique.id     <- 57
analysis.name          <- 'Savani.3a'
analysis.type          <- 4
analysis.type.name     <- 'study_by_order'
analysis.type.groups   <- 'study.order'
Nmin.raw               <- 30
Nmin.cond              <- 15
subset                 <- 'all'
onlineTables           <- TRUE
...
analysis.root          <- file.path(OSFdata.root,study.description,analysis.name,'By Order')
...
ML2.df <- ML2.df  %>% dplyr::select(1,6,30,31,37,38,45,46,53,54,61,62,69,70,76,77,83,84,95,96,102,103,110,111,118,119,126,127,134,135,141,142,148,149,804,903,904,905,906,907,908,909,910,911,912,913,914,937,938,939) %>% dplyr::filter(is.character(source))
...
# To see the function code type:varfun.Savani.1, or lookup in manylabRs_SOURCE.R
ML2.var[[g]] <- varfun.Savani.1(ML2.sr[[g]])
...
stat.test   <- try.CATCH(with(ML2.var[[g]],summary(lme4::glmer(Response ~ Importance*Condition + (1|uID/trialID), family = binomial('logit'), nAGQ=0))))
