...
##
## Intuitive Reasoning (Norenzayan et al., 2002): Norenzayan_1_study_by_order_all
##
...
ML2.df <- ML2.df  %>% dplyr::select(1,6,435,440,445,450,455,460,465,470,475,480,485,490,495,500,505,510,515,520,525,530,535,540,545,550,555,560,565,570,575,580,585,590,595,600,605,610,615,620,625,630,804,903,904,905,906,907,908,909,910,911,912,913,914,937,938,939) %>% dplyr::filter(is.character(source))
...
# To see the function code type:varfun.Norenzayan.1, or lookup in manylabRs_SOURCE.R
...
stat.test   <- try.CATCH(with(ML2.var[[g]],t.test(x = Belong, y = Similar, conf.level=stat.params$conf.level, var.equal = stat.params$var.equal, alternative = stat.params$alternative)))
