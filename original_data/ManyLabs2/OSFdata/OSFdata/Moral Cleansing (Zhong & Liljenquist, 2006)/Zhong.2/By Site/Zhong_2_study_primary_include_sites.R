...
##
## Moral Cleansing (Zhong & Liljenquist, 2006): Zhong_2_study_primary_include_sites
##
...
analysis.unique.id     <- 66
analysis.name          <- 'Zhong.2'
analysis.type          <- 3
...
ML2.df <- ML2.df  %>% dplyr::select(1,6,227,228,229,230,231,232,233,234,235,236,804,903,904,905,906,907,908,909,910,911,912,913,914,933,934,937,938,939) %>% dplyr::filter(is.character(source))
...
# To see the function code type:varfun.Zhong.2, or lookup in manylabRs_SOURCE.R
ML2.var[[g]] <- varfun.Zhong.2(ML2.sr[[g]])
...
stat.test   <- try.CATCH(with(ML2.var[[g]],summary(lmerTest::lmer(Response ~ Condition * Product + (1|uID)))))
