...
##
## Affect & Risk (Rottenstreich & Hsee, 2001): Rottenstreich_1_study_by_order_all
##
...
study.description      <- 'Affect & Risk (Rottenstreich & Hsee, 2001)'
analysis.unique.id     <- 16
analysis.name          <- 'Rottenstreich.1'
analysis.type          <- 4
...
ML2.df <- ML2.df  %>% dplyr::select(1,6,13,18,520,521,522,523,524,525,526,527,528,529,530,531,534,535,536) %>% dplyr::filter(is.character(source))
...
# To see the function code type:varfun.Rottenstreich.1, or lookup in manylabRs_SOURCE.R
ML2.var[[g]] <- varfun.Rottenstreich.1(ML2.sr[[g]])
...
stat.test   <- try.CATCH(with(ML2.var[[g]],fisher.exact(Condition,Response)))
