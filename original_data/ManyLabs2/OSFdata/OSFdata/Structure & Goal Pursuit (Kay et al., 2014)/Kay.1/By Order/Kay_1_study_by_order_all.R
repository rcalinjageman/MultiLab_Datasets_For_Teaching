...
##
## Structure & Goal Pursuit (Kay et al., 2014): Kay_1_study_by_order_all
##
...
ML2.df <- ML2.df  %>% dplyr::select(1,6,102,103,104,121,122,123,520,521,522,523,524,525,526,527,528,529,530,531,534,535,536) %>% dplyr::filter(is.character(source))
...
# To see the function code type:varfun.Kay.1, or lookup in manylabRs_SOURCE.R
...
stat.test   <- try.CATCH(with(ML2.var[[g]],t.test(x = Order, y = DisOrder, conf.level=stat.params$conf.level, var.equal = stat.params$var.equal, alternative = stat.params$alternative)))
