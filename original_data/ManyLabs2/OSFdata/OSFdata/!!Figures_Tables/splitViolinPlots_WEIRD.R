## WEIRD split VIOLIN PLOTS - MANYLABS 2
#
...
Slabels <- c("less WEIRD","WEIRD","Multi",".")
...
outlistAll$source.WEIRD.f <- relevel(outlistAll$source.WEIRD.f,ref="less WEIRD")
...
for(o in unique(oriEffects$study.analysis.ori)){
 ...
 outlistFigure2[ID1&ID2&ID3,241:300] <- oriEffects[oriEffects$study.analysis.ori%in%o,cID]
}
...
# rio::export(outlistFigure2, paste0(dir.in,"Data_Figure_SPLIT.rds"))
# rio::export(outlistFigure2, paste0(dir.in,"Data_Figure_SPLIT.csv"))


# make plot data frame ----
#df <- rio::import(paste0(dir.in,"Data_Figure_SPLIT.csv"))
...
pdat$splitv <- relevel(pdat$splitv, ref = "less WEIRD")
...
for(sl in levels(pdat$study.labels)){
  ...
  mxLab <- ifelse(tmp$splitv[mxID]%in%splits[1],levels(splits)[2],levels(splits)[1])
  mnLab <- ifelse(tmp$splitv[mnID]%in%splits[2],levels(splits)[1],levels(splits)[2])
  pdat <- add_row(pdat,
...
for(n in seq_along(labs)){
  esID    <- df$study.labels%in%labs[n]
  meansID <- means$study.labels%in%df$study.description[esID]

  if(any(meansID&means$splitv%in%Slabels[1])){
    means$oriWEIRD[meansID&means$splitv%in%Slabels[1]]  <- Slabels[1]
    means$oriES[meansID&means$splitv%in%Slabels[1]]     <- df$ori.ESCI.r[esID&(df$source.Weird==0)]%00%NA
  }
  if(any(meansID&means$splitv%in%Slabels[2])){
    means$oriWEIRD[meansID&means$splitv%in%Slabels[2]]  <- Slabels[2]
    means$oriES[meansID&means$splitv%in%Slabels[2]]     <- df$ori.ESCI.r[esID&df$source.Weird==1]%00%NA
  }
}
...
cols <- c("less WEIRD" = credL, "WEIRD" = cblueL, "Mixed"="#5aae61")
...
list(pdat,sums,means,xbreaks,xlabels,)
...
g2b <- ggplot(pdat, aes(offset_dens, loc, group = interaction(pdat[['study.labels']], pdat[['splitv']])), colour = splitv) +
  ...
  theme(legend.position = "top",
        ...
        panel.grid.major.y = element_blank(), #element_line(colour="grey70"),
...
figname <- paste0("ML2_SplitViolin_MEANsort_QonTop_2ndAxis_with_line_WEIRDsolit",as.Date(now()))
ggsave(plot = g2b, filename = paste0(outdir,"/",figname,".eps"),width=10, units = "in",dpi=600)
ggsave(plot = g2b, filename = paste0(outdir,"/",figname,".pdf"),width=10, units = "in",dpi=600)
ggsave(plot = g2b, filename = paste0(outdir,"/",figname,".png"),width=10, units = "in",dpi=600)
ggsave(plot = g2b, filename = paste0(outdir,"/",figname,".tiff"),width=10, units = "in",dpi=600)
