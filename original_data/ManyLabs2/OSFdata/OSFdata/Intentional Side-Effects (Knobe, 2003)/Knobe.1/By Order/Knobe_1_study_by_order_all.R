...
##
## Intentional Side-Effects (Knobe, 2003): Knobe_1_study_by_order_all
##
...
ML2.df <- ML2.df  %>% dplyr::select(1,6,179,187,804,903,904,905,906,907,908,909,910,911,912,913,914,937,938,939) %>% dplyr::filter(is.character(source))
...
# To see the function code type:varfun.Knobe.1, or lookup in manylabRs_SOURCE.R
...
stat.test   <- try.CATCH(with(ML2.var[[g]],t.test(x = Harm, y = Help, conf.level=stat.params$conf.level, var.equal = stat.params$var.equal, alternative = stat.params$alternative)))
