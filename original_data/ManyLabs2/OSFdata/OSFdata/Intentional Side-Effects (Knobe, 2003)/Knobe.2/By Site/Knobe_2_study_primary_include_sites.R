...
##
## Intentional Side-Effects (Knobe, 2003): Knobe_2_study_primary_include_sites
##
...
analysis.unique.id     <- 80
analysis.name          <- 'Knobe.2'
analysis.type          <- 3
...
ML2.df <- ML2.df  %>% dplyr::select(1,6,180,188,804,903,904,905,906,907,908,909,910,911,912,913,914,937,938,939) %>% dplyr::filter(is.character(source))
...
# To see the function code type:varfun.Knobe.2, or lookup in manylabRs_SOURCE.R
ML2.var[[g]] <- varfun.Knobe.2(ML2.sr[[g]])
...
stat.test   <- try.CATCH(with(ML2.var[[g]],t.test(x = Blame, y = Praise, conf.level=stat.params$conf.level, var.equal = stat.params$var.equal, alternative = stat.params$alternative)))
