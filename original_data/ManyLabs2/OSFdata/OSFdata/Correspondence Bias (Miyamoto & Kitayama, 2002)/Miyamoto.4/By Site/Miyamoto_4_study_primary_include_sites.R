...
##
## Correspondence Bias (Miyamoto & Kitayama, 2002): Miyamoto_4_study_primary_include_sites
##
...
analysis.unique.id     <- 25
analysis.name          <- 'Miyamoto.4'
analysis.type          <- 3
...
ML2.df <- ML2.df  %>% dplyr::select(1,6,49,56,57,77,84,85,520,521,522,523,524,525,526,527,528,529,530,531,534,535,536) %>% dplyr::filter(is.character(source))
...
# To see the function code type:varfun.Miyamoto.2, or lookup in manylabRs_SOURCE.R
ML2.var[[g]] <- varfun.Miyamoto.2(ML2.sr[[g]])
...
stat.test   <- try.CATCH(with(ML2.var[[g]],summary(lm(Attitude ~ Constraint + Moderator * Condition))))
