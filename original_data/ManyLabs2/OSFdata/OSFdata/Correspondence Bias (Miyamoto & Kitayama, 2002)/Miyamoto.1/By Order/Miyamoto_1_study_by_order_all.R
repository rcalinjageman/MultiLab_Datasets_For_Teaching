...
##
## Correspondence Bias (Miyamoto & Kitayama, 2002): Miyamoto_1_study_by_order_all
##
...
study.description      <- 'Correspondence Bias (Miyamoto & Kitayama, 2002)'
analysis.unique.id     <- 22
analysis.name          <- 'Miyamoto.1'
analysis.type          <- 4
...
ML2.df <- ML2.df  %>% dplyr::select(1,6,49,56,77,84,520,521,522,523,524,525,526,527,528,529,530,531,534,535,536) %>% dplyr::filter(is.character(source))
...
# To see the function code type:varfun.Miyamoto.1, or lookup in manylabRs_SOURCE.R
ML2.var[[g]] <- varfun.Miyamoto.1(ML2.sr[[g]])
...
stat.test   <- try.CATCH(with(ML2.var[[g]],summary(lm(Attitude ~ Constraint + Condition))))
