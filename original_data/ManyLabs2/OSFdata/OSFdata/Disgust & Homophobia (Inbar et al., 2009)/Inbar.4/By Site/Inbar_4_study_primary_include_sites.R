...
##
## Disgust & Homophobia (Inbar et al., 2009): Inbar_4_study_primary_include_sites
##
...
analysis.unique.id     <- 35
analysis.name          <- 'Inbar.4'
analysis.type          <- 3
...
ML2.df <- ML2.df  %>% dplyr::select(1,6,164,173,520,521,522,523,524,525,526,527,528,529,530,531,534,535,536) %>% dplyr::filter(is.character(source))
...
# To see the function code type:varfun.Inbar.2, or lookup in manylabRs_SOURCE.R
ML2.var[[g]] <- varfun.Inbar.2(ML2.sr[[g]])
...
stat.test   <- try.CATCH(with(ML2.var[[g]],t.test(x = SameKiss, y = DiffKiss, conf.level=stat.params$conf.level, var.equal = stat.params$var.equal, alternative = stat.params$alternative)))
