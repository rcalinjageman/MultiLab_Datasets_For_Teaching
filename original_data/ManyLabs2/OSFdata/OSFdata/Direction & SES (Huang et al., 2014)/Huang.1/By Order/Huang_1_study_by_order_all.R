...
##
## Direction & SES (Huang et al., 2014): Huang_1_study_by_order_all
##
...
ML2.df <- ML2.df  %>% dplyr::select(1,6,149,150,156,157,520,521,522,523,524,525,526,527,528,529,530,531,534,535,536) %>% dplyr::filter(is.character(source))
...
# To see the function code type:varfun.Huang.1, or lookup in manylabRs_SOURCE.R
...
stat.test   <- try.CATCH(with(ML2.var[[g]],t.test(x = High, y = Low, conf.level=stat.params$conf.level, var.equal = stat.params$var.equal, alternative = stat.params$alternative)))
